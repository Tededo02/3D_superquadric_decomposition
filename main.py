...
def create_and_estimate_supq():
    ...
    sampled_points_noisy, normals_sp_noisy = samp.sampling_sq_noisy(list_mesh, n_points=4000, noise_std=NOISE_STD, clip_k=3.0, seed=42)
    sampled_points_random, _ = samp.sampling_sq_random(list_mesh, n_points=5000, seed=42)
    ...
    if algorithm == "ls":
        ...
        graph_radius = 0.08
        models, inliers_masks, total_best_mss_used = gair_ransac(sampled_points, normals, threshold=THRESHOLD, max_models=len(list_mesh),max_iterations=10,inner_iterations=40, radius=graph_radius,use_normal_coherence=True,m_neighbors=4)
        if not models:
