...
def create_and_estimate_supq():
    ...
    res_chamfers          = {a: {s: [] for s in scale_factors} for a in algorithms}
    res_hausdorff         = {a: {s: [] for s in scale_factors} for a in algorithms}
    res_misclassification = {a: {s: [] for s in scale_factors} for a in algorithms}
    res_runtime           = {a: {s: [] for s in scale_factors} for a in algorithms}
    ...
    sampled_points_outliers, normals_sp_outliers = samp.sampling_outliers(list_mesh, n_out=3000, margin=0.10, mode="uniform", seed=42)
    
    for step in scale_factors:
        threshold = step * NOISE_STD
        for algo in algorithms:
            for trial in range(ITERATIONS_STEP):
                print(f"Scale Factor: {step}; Algo: {algo}")
            ...
            print(f"[avg] scale={step} algo={algo}  chamfer={chamfer:.4f}  hausdorff={hausdorff:.4f}  runtime={runtime:.2f}s")

    import matplotlib
    matplotlib.use("Agg")
    import matplotlib.pyplot as plt
    output_dir = Path(__file__).resolve().parent / "artifacts" / "1_scale_factor"
    ...
    for metric_name, res in [("chamfer", res_chamfers), ("hausdorff", res_hausdorff), ("accuracy", res_misclassification), ("runtime_s", res_runtime)]:
        ...
        for algo in algorithms:
            means = np.array([np.nanmean(res[algo][s]) for s in scale_factors])
            stds  = np.array([np.nanstd(res[algo][s])  for s in scale_factors])
        ...
        ax.set_title(f"Scale factor vs {metric_name}")
    ...
    with csv_path.open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=["algo", "scale_factor", "trial", "chamfer", "hausdorff", "accuracy", "runtime_s"])
        writer.writeheader()
        for algo in algorithms:
            for s in scale_factors:
                for i in range(len(res_chamfers[algo][s])):
                    writer.writerow({
                        "algo": algo, "scale_factor": s, "trial": i,
