...
def create_and_estimate_supq():
    ...
    noise_values = list(np.round(np.arange(0.5, 0.5+0.05, 0.05), 2))
    gt_meshes = [supmesh.superquadric_mesh(p) for p in gt_params]
    algorithms = ["gair-ransac", "ransac"]
    res_chamfers          = {a: {s: [] for s in noise_values} for a in algorithms}
    res_hausdorff         = {a: {s: [] for s in noise_values} for a in algorithms}
    res_misclassification = {a: {s: [] for s in noise_values} for a in algorithms}
    res_runtime           = {a: {s: [] for s in noise_values} for a in algorithms}
    
    for step in noise_values:
        noise_std = step
        threshold = max(THRESHOLD_SCALE * noise_std, 0.5)
        for algo in algorithms:
            for trial in range(ITERATIONS_STEP):
                print(f"Noise: {noise_std}; Algo: {algo}; Trial: {trial}")
                seed = trial * 1000 + int(noise_std * 100)
                ...
                sampled_points_outliers, normals_sp_outliers = samp.sampling_outliers(gt_meshes, n_out=0, margin=0.10, mode="uniform", seed=seed + 10000)
                ...
                res_runtime[algo][step].append(elapsed)
                ...
                res_chamfers[algo][step].append(chamfer_distance(sampled_points_random,sample_from_supq_estimated))
                print(f"symmetric chamfer distance from 2 exact mesh = {res_chamfers[algo][step][-1]:.4f}")
                ...
                res_hausdorff[algo][step].append(max(d_gt_to_est.max(), d_est_to_gt.max()))
                print(f"Hausdorff distance = {res_hausdorff[algo][step][-1]:.4f}")
                
                # accuracy: correctly classified / total (GT labels known from sampling)
                ...
                res_misclassification[algo][step].append(accuracy)
            ...
            hausdorff = np.mean(res_hausdorff[algo][step])
            chamfer = np.mean(res_chamfers[algo][step])
            runtime = np.mean(res_runtime[algo][step])
            print(f"[avg] noise={step} algo={algo}  chamfer={chamfer:.4f}  hausdorff={hausdorff:.4f}  runtime={runtime:.2f}s")

    import matplotlib
    matplotlib.use("Agg")
    import matplotlib.pyplot as plt
    output_dir = Path(__file__).resolve().parent / "artifacts" / "2_noise"
    output_dir.mkdir(parents=True, exist_ok=True)
    scale_arr = np.array(noise_values)
    ...
    for metric_name, res in [("chamfer", res_chamfers), ("hausdorff", res_hausdorff), ("accuracy", res_misclassification), ("runtime_s", res_runtime)]:
        ...
        for algo in algorithms:
            means = np.array([np.nanmean(res[algo][s]) for s in noise_values])
            stds  = np.array([np.nanstd(res[algo][s])  for s in noise_values])
            ax.plot(scale_arr, means, marker="o", linewidth=2, label=algo, color=colors_algo[algo])
            ax.fill_between(scale_arr, means - stds, means + stds, alpha=0.15, color=colors_algo[algo])
        ax.set_xlabel("noise_std")
        ax.set_ylabel(metric_name)
        ax.set_title(f"Noise vs {metric_name}")
        ax.grid(True, linestyle="--", linewidth=0.5, alpha=0.5)
        ax.legend()
        fig.tight_layout()
        path = output_dir / f"scale_factor_vs_{metric_name}.png"
    ...
    with csv_path.open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=["algo", "noise_std", "trial", "chamfer", "hausdorff", "accuracy", "runtime_s"])
        writer.writeheader()
        for algo in algorithms:
            for s in noise_values:
                for i in range(len(res_chamfers[algo][s])):
                    writer.writerow({
                        "algo": algo, "noise_std": s, "trial": i,
